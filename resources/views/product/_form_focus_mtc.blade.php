    ...
    <div class="modal-content">
      <div class="block block-themed block-transparent mb-0">
        <div class="block-header bg-gd-sun p-10">
          <h3 class="block-title"><i class="fa fa-plus"></i> {{ $type == '' ? 'Add' : ucfirst($type) }} Product Fokus MD</h3>
      ...
      <form action="{{ $action }}" method="post" id="{{ $type }}Form">
        ...
        <div class="block-content">
          ...
          <div class="form-group">
            <label>Channel</label>
            <select class="{{$type}}-js-select2 form-control" style="width: 100%" id="{{$type}}Channel" name="id_channel">
              <option value="" disabled selected>Choose your Channel</option>
              @foreach(App\Channel::get() as $data)
              <option value="{{ $data->id }}">{{ $data->name }} </option>
              @endforeach
            </select>
          </div>
...
<script type="text/javascript">
  ...
  function editModal(json) {
    ...
    $('#{{$type}}Form').attr('action', "{{ url('/product/fokus-mtc/update') }}/"+json.id);
