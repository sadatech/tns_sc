...
// options[sample_data]  -> sample input with the table
//################################

$title        = ucwords(str_replace('_',' ',$name));
$id           = str_replace(' ','',$title);
$template_url = route($template_url);
$sample_data  = isset( $options['sample_data'] ) ? $options['sample_data'] : "";
...
{{-- <script type="text/javascript"> --}}
    function importModal(buttonId) {
        $('#'+buttonId).attr('data-toggle','modal');
        $('#'+buttonId).attr('href','#importModal{{ $id }}');
    }
{{-- </script> --}}
@endpush